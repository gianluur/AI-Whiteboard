...
class HandDetector:
  ...
  def get_finger_status(self, landmarks) -> Dict[str, bool]:
    """Get the status of all fingers"""
    fingers = {
    ...
    return {
      finger_name: self.is_finger_up(landmarks, tip, dip, pip)
      for finger_name, (tip, dip, pip) in fingers.items()
    }
...
class HandDrawingApp:
  ...
  def process_frame(self, frame: np.ndarray):
    ...
    cv.putText(frame, f"Thickness: {self.state.thickness}", (2, 35), cv.FONT_HERSHEY_SIMPLEX, 0.85, ColorPalette.WHITE, 2)
    return frame
